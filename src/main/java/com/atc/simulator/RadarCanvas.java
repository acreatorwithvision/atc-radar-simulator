...
public class RadarCanvas extends Canvas {
    ...
    public static final double SWEEP_SPEED = 0.012;

    private double sweepAngle = 0.0;
    private AnimationTimer animationTimer;

    // Live reference to aircraft collection (set by ATCApplication)
    private volatile Collection<Aircraft> aircraftCollection = Collections.emptyList();
    private final AircraftRenderer aircraftRenderer = new AircraftRenderer();
    ...
    /** Called by ATCApplication to inject live aircraft data. */
    public void setAircraftCollection(Collection<Aircraft> aircraft) {
    ...
    protected void render() {
        ...
        // ── Aircraft layer ────────────────────────────────────────────────
        clipToRadarCircle(gc);
        aircraftRenderer.renderAll(gc, aircraftCollection, sweepAngle);
        gc.restore();
        // ─────────────────────────────────────────────────────────────────
    ...
    private void clipToRadarCircle(GraphicsContext gc) {
        gc.save();
        gc.beginPath();
        gc.arc(CENTER_X, CENTER_Y, MAX_RADIUS, MAX_RADIUS, 0, 360);
        gc.closePath();
        gc.clip();
    }
    ...
    private void drawRangeRings(GraphicsContext gc) {
        ...
        for (int i = 1; i <= RANGE_RINGS; i++) {
            ...
            gc.fillText((i * 25) + "NM", CENTER_X + 4, CENTER_Y - r + 12);
        }
    }
    ...
    private void drawCrosshairs(GraphicsContext gc) {
        ...
        gc.strokeLine(CENTER_X - MAX_RADIUS, CENTER_Y, CENTER_X + MAX_RADIUS, CENTER_Y);
        gc.strokeLine(CENTER_X, CENTER_Y - MAX_RADIUS, CENTER_X, CENTER_Y + MAX_RADIUS);
    ...
    private void drawCompassRose(GraphicsContext gc) {
        ...
        for (int i = 0; i < 360; i += 10) {
            ...
            gc.strokeLine(
                ...
                CENTER_X + MAX_RADIUS   * Math.cos(rad),
                CENTER_Y + MAX_RADIUS   * Math.sin(rad)
            );
        }

        for (int i = 0; i < 8; i++) {
            double rad = Math.toRadians(i * 45 - 90);
            gc.setFill(i % 2 == 0 ? COLOR_TEXT : COLOR_TEXT_DIM);
            gc.setFont(Font.font("Courier New", i % 2 == 0 ? 12 : 10));
            gc.fillText(cardinals[i],
                CENTER_X + labelR * Math.cos(rad),
                CENTER_Y + labelR * Math.sin(rad) + 4);
        }
    }
